...
lb_threshold = 0.4                                                      # the threshold for recognizing the attention map as mask 
...
class ProT2IPipeline(StableDiffusionPipeline):
     ...
     self,
        ...
        **kwargs,
        ...
        if cross_attention_kwargs.get("nps", None):
            ...
            prompt, lb_words_list = _get_nps_and_spans(prompt)
        ...
        #6.1 add kwarges, no involved in IP-Adapter
        ...
        if cross_attention_kwargs.get("set_controller", None):
            ...
            self.controller_list = self._get_controller_list(prompt, lb_words_list) 

        self.register_attention_control(self.controller_list)  # add attention controller
        ...
        return (StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept),self.controller_list)

    def register_attention_control(self, controller_list):
        attn_procs = {}
        cross_att_count = 0
        for name in self.unet.attn_processors.keys():
            ...
            if name.startswith("mid_block"):
                ...
                continue
            cross_att_count += 1
            attn_procs[name] = P2PAttnProcessor(controller_list=controller_list, place_in_unet=place_in_unet, do_cfg=self.do_classifier_free_guidance)

        self.unet.set_attn_processor(attn_procs)
        for controller in controller_list:
            controller.num_att_layers = cross_att_count

    # TODO: add the logic of generate the controller chain automatically 
